...
$(document).ready(function() {
	...
	google_maps_api_key = 'AIzaSyC36Dibv-Xy2F5Uuvz3DuWQQuNVsiIvlTE';
...
function calculateValue() {
	...
	var requestUrl = "https://developer.nrel.gov/api/pvwatts/v5.json?api_key=" + nrel_api_key + "&lat=" + lat + "&lon=" + lon + "&timeframe=monthly&system_capacity=" + size_premimum + "&module_type=1&losses=14&array_type=1&tilt=" + tilt + "&azimuth=" + azimuth + "&callback=?";
	...
	requestUtilityCompanies = "https://developer.nrel.gov/api/census_rate/v3.json?api_key=" + nrel_api_key + "&region=block&id=101&lat=" + lat + "&lon=" + lon;
...
nrel_api_key = 'B3eeEuXR7zVF7Hw8zkUmwp6tv7MEBrlunhW6ny49';
...
function show_utility_companies() {
	...
	var requestUtilityCompanies = "https://developer.nrel.gov/api/census_rate/v3.json?api_key=" + nrel_api_key + "&region=block&id=101&address=" + address;
	...
	function show_average_electrcity_tariffs() {
		...
		city = city.split(' ').join('+');
		var address = city + "+" + document.getElementById('state').value;
		var requestAvgElectTariffs = "https://developer.nrel.gov/api/utility_rates/v3.json?api_key=" + nrel_api_key + "&address=" + address;
		...
		function pvwatts_irradiance() {
			...
			var tilt = parseInt(document.getElementById('tilt').value);
			var azimuth = parseInt(document.getElementById('azimuth').value);
			var requestUrl = "https://developer.nrel.gov/api/pvwatts/v5.json?api_key=" + nrel_api_key + "&address=" + address + "&timeframe=monthly&system_capacity=" + size + "&module_type=" + module_type + "&losses=14&array_type=" + array_type + "&tilt=" + tilt + "&azimuth=" + azimuth + "&callback=?";
			$.getJSON(requestUrl, {},
				function(data) {
			...
			function pvwatts() {
				...
				function show_incentives() {
					...
					city = city.split(' ').join('+');
					var address = city + "+" + document.getElementById('state').value;
					var requestDsire = "https://developer.nrel.gov/api/energy-incentives/v1.json?api_key=" + nrel_api_key + "&address=" + address + "&incentive_types=solar_incentives&callback=?";
					var request = $.ajax({
						url: requestDsire,
						cache: false
					});
					request.always(function(jqXHR) {
						var data = JSON.parse(jqXHR.responseText.substring(1, jqXHR.responseText.length - 1).substring(1, jqXHR.responseText.length));
						if(typeof data == 'object' && data.outputs){
					...
					var requestCos = 'https://data.ct.gov/resource/jh52-2msw.json?city=New%20Britain&state=CT&active=1&$where=credentialtype%20in(%27HIC%27,%27HIS%27)';
					var request2 = $.ajax({
						url: requestCos,
						cache: false
					});
					request2.always(function(data) {

						if(typeof data == 'object' && data.length){
						document.getElementById('list_of_cos').innerHTML = "<h2>List of Local Contractors:</h2><br>";
						var i = 0;
						while (data[i]) {
							document.getElementById('list_of_cos').innerHTML += "<div><strong> Business Name:</strong> " + data[i].businessname + "<br />" + "<strong> Address:</strong> " + data[i].address + " <hr/> </div>";
							i++;
						}
					}
					});
				...
				function avg_costperwatt() {
					...
					var requestSystemCost = "https://developer.nrel.gov/api/solar/open_pv/installs/summaries?api_key=" + nrel_api_key + "&county=" + county + "&minsize=" + minsize + "&maxsize=" + maxsize;
